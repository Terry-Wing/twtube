...
* __OUTPUT_TEMPLATE__: The template for the filenames of the downloaded videos, formatted according to [this spec](https://github.com/yt-dlp/yt-dlp/blob/master/README.md#output-template). Defaults to `%(uploader,channel,creator&{} - |)s%(upload_date>%Y-%m-%d&{} - |)s%(title)s.%(ext)s`, i.e. `author - date - title.ext`, with the author and date segments dropped when the site does not provide them. The resolution/framerate tag (see `FILENAME_VIDEO_INFO` below) is appended after the download and is not part of this template.
...
* __FILENAME_VIDEO_INFO__: Whether to append the finished file's real resolution and framerate to its name, e.g. `author - 2026-09-16 - title [2160x3840 60fps].mp4`. The values are read from the merged file with `ffprobe` once post-processing finishes, so they describe the actual output rather than the pre-download metadata; portrait videos with a rotation flag report their displayed dimensions. The tag is omitted for downloads without a video stream (audio, captions, thumbnails), when the values cannot be read, and for chapter-split downloads (which produce several files). Applies to every platform, including the native Douyin downloader. Defaults to `true`.
